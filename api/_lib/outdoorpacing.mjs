...
export function buildCandidateTiming({ candidateId, examinerId, focusEvents, openedAt, submittedAt, budgets, budgetKnown = true }) {
  const dwell = accumulateDwell(focusEvents, { endedAt: submittedAt });
  // Union of budgeted and actually-visited sections. Driving this from the budget alone made
  // measured time vanish whenever the two disagreed - a renamed section, an either/or variant, or a
  // level that could not be resolved - which reads as "nothing happened" rather than as a gap.
  const sections = Array.from(new Set([...Object.keys(budgets || {}), ...Object.keys(dwell.perSection)]));
  const rows = sections
  ...
  return {
    ...
    // Distinguishes "kept exactly to a zero budget" from "we could not work out a budget at all".
    // Without this a run whose level could not be resolved reads as a huge overrun, which would be
    // a lie about the examiner.
    budgetKnown: Boolean(budgetKnown) && plannedTotal > 0,
    sections: rows,
...
export function aggregateByExaminer(timings) {
  ...
  for (const timing of timings) {
    if (!timing.examinerId || !timing.complete || !timing.budgetKnown) continue;
