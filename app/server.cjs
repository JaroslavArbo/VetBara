...
const adminSessionToken = process.env.VETBARA_ADMIN_TOKEN || crypto.randomBytes(24).toString('hex');
...
function resetHtml() { return `<!doctype html><meta charset="utf-8"><title>VetBara reset</title><body style="font-family:system-ui;padding:32px"><h1>VetBara cache reset</h1><p>Resetting service workers and caches...</p><pre id="out"></pre><script>(async()=>{const out=document.getElementById('out');function log(s){out.textContent+=s+'\n'}; if('serviceWorker' in navigator){const regs=await navigator.serviceWorker.getRegistrations(); log('service workers: '+regs.length); for(const r of regs){await r.unregister(); log('unregistered '+(r.scope||''));}} if(window.caches){const keys=await caches.keys(); log('caches: '+keys.length); for(const k of keys){await caches.delete(k); log('deleted '+k);}} log('done - opening VetBara'); setTimeout(()=>location.href='/?role=${mode === 'admin' ? 'Admin&token=' + adminSessionToken : 'Centre'}&reset=1',800);})();</script></body>`; }
...
server.listen(port, '0.0.0.0', () => {
  const adminQuery = `?role=Admin&token=${encodeURIComponent(adminSessionToken)}`;
