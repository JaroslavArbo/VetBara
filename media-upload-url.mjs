...
// Never trust the client with a path segment. Besides stripping unsafe characters this peels a
// pasted link down to its id: a Centre link used as an exam id used to be slugged whole, so the
// access token inside it ended up embedded in the storage path and in exam_media - a secret must
// never become part of an identifier. Also capped, because those paths grew absurdly long.
function sanitizeSegment(value, fallback) {
  let raw = String(value ?? "").trim();
  if (/^https?:\/\//i.test(raw)) {
    try {
      const url = new URL(raw);
      raw = url.searchParams.get("id") || url.searchParams.get("examId") || url.pathname.replace(/\//g, "-");
    } catch { /* fall through and slug what we have */ }
  }
  // Defence in depth: drop anything that still looks like a bare access token.
  raw = raw.replace(/\b(?:CENTRE|VETBARA)-[A-Za-z0-9_-]{8,}/gi, "");
  const cleaned = raw.replace(/[^a-zA-Z0-9._-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
  return cleaned.slice(0, 80).replace(/-+$/, "") || fallback;
}
